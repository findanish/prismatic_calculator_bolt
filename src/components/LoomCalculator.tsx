...
import { loomRecipes } from '../data/loomRecipes';
import type { CalculationResult } from '../types';
import { ChevronRight } from 'lucide-react';

const getItemColor = (itemName: string, isDark: boolean = false): string => {
  if (itemName.includes('Linen')) return '#4299e1';  // Blue
  if (itemName.includes('Sateen')) return '#f56565'; // Red
  if (itemName.includes('Silk') && !itemName.includes('Infused')) return '#ed8936'; // Orange
  if (itemName.includes('Infused Silk')) return '#805ad5'; // Purple
  if (itemName.includes('Phoenixweave')) return '#dd6b20'; // Dark Orange
  if (itemName.includes('Spinweave')) return '#4c51bf'; // Indigo
  if (itemName.includes('Prismatic')) return '#8B4513'; // Dark Brown/Yellow
  if (itemName.includes('Wireweave')) return '#718096'; // Gray
...
function calculateTotalMaterials(recipeName: string, quantity: number, level: number = 0, cache = new Map<string, number>()): { item: string; quantity: number; level: number }[] {
  const recipe = loomRecipes.find(r => r.value === recipeName || r.name === recipeName);
  if (!recipe) return [];
  ...
  recipe.ingredients.forEach(ingredient => {
    const subRecipe = loomRecipes.find(r => r.name === ingredient.item);
  ...
  for (const [key, value] of cache.entries()) {
    if (key.startsWith('recipe:')) {
      materials.push({
        ...
        quantity: value,
        level: loomRecipes.find(r => r.name === key.replace('recipe:', ''))?.tier || 0
      });
    }
  }
...
function aggregateBaseResources(breakdown: { item: string; quantity: number; level: number }[]) {
  ...
  const wireweaveQuantity = calculateTotalReagents(breakdown);
  
  breakdown.forEach(item => {
    const isBaseResource = !loomRecipes.find(r => r.name === item.item);
    if (isBaseResource && item.item !== 'Wireweave') {
  ...
  if (wireweaveQuantity > 0) {
    materials.push({
      item: 'Wireweave',
      quantity: wireweaveQuantity
    });
  }
  ...
  return breakdown
    .sort((a, b) => {
      const getRecipeLevel = (itemName: string): number => {
        if (itemName === 'Linen') return 7;
        if (itemName === 'Sateen') return 6;
        if (itemName === 'Silk') return 5;
        if (itemName === 'Infused Silk') return 4;
        if (itemName === 'Phoenixweave') return 3;
        if (itemName === 'Spinweave Cloth') return 2;
        if (itemName === 'Prismatic Cloth') return 1;
        return 0;
      };
...
function calculateTotalReagents(breakdown: { item: string; quantity: number; level: number }[]): number {
  let totalWireweave = 0;
  
  breakdown.forEach(item => {
    const recipe = loomRecipes.find(r => r.name === item.item);
    if (recipe) {
      const wireweaveIngredient = recipe.ingredients.find(ing => ing.item === 'Wireweave');
      if (wireweaveIngredient) {
        totalWireweave += wireweaveIngredient.quantity * item.quantity;
      }
    }
  });
  
  return totalWireweave;
}

export function LoomCalculator() {
  const [selectedItem, setSelectedItem] = useState(loomRecipes[0].value);
  const [quantity, setQuantity] = useState(1);
  const [result, setResult] = useState<CalculationResult | null>(null);

  const calculateResources = (e: React.FormEvent) => {
    e.preventDefault();
    const recipe = loomRecipes.find(r => r.value === selectedItem);
    if (!recipe) return;
            ...
            <div className="space-y-6">
              ...
              <div className="space-y-4">
                ...
                {groupMaterialsByType(result.breakdown)
                  .filter(item => loomRecipes.find(r => r.name === item.item))
                  .map((item, index) => {
                    const recipe = loomRecipes.find(r => r.name === item.item);
